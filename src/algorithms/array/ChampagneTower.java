...
public class ChampagneTower {
    ...
    public double champagneTower(int poured, int query_row, int query_glass) {
        ...
        dp[0][0] = (double)poured; //Assume each glass catches all the champagne before dispensing it down
        for(int r = 0; r <= query_row; r++) {
            for(int c = 0; c <= r; c++) {
                double flows = (dp[r][c] - 1.0) / 2; //Each glass can only hold 1, the rest flows to the next row
                if(flows > 0) {
                    dp[r+1][c] += flows;
                    dp[r+1][c+1] += flows;
                }
